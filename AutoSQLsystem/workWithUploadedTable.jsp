...
<%
        ...
        {
        	//fetched table name from server according to user session stored email id 
            LinkedHashMap<String, String> tableNames = table.getUploadedTableList(email);
        ...
        <p>
            ...
            %>
                <label class="container">
                        <input type=checkbox name=tableName value=<%=entry.getValue().substring(0, entry.getValue().lastIndexOf("."))+entry.getKey()%> onchange="tap(this)"><%=entry.getValue()%>
...
function confirm()
{
    if(count > 0)
    {
        document.attributeForm.action="viewTable2";
        return true;
    }
    else
    {
